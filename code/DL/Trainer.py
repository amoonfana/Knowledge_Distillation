...
def fit(train_loader, val_loader, model, loss_fn, optimizer, scheduler, n_epochs, patience, device, save_path, log_interval, acc_val_global_best):
    time_train_total = 0
    time_val_avg = 0
    best_val_acc = 0
    es_cnt = 0
    ...
    #         if best_val_acc > acc_val_global_best:
...
def train_epoch(train_loader, model, loss_fn, optimizer, device, log_interval):
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        #Input-->model-->output-->loss_fn-->loss
        output = model(data)
