...
def draw_cd_diagram(df_perf=None, alpha=0.05, title=None, labels=False, save_dir=None):
    """
...
def cd_diagram(csv_dir, fig_dir, col):
    csv_list = []
    ...
    for csv in csv_list:
        ...
        classifier_name = pd.DataFrame(columns = ["classifier_name"], data = [csv[:-4]]*result.shape[0])
        dataset_val_acc = result[["dataset", col]]
    ...
    draw_cd_diagram(df_perf=results, title='Accuracy', labels=True, save_dir=fig_dir)

if __name__ == "__main__":
    cd_diagram("../../results", "../../diagrams/cd-diagram.png", "acc_mean")
    plt.show()